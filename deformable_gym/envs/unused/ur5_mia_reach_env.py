...
class UR5MiaReachEnv(BaseBulletEnv):
    ...
    def __init__(self, gui=True, real_time=False):
        ...
        lower = np.concatenate([np.array([-1.25, -0.25, 0.75]), np.zeros(4),
                                ...
                                limits[0][6:], np.array([-5, -5, -5])], axis=0)
        upper = np.concatenate([np.array([-0.75, 0.25, 1.25]), np.ones(4),
                                np.array([-0.5, 0.5, 1.5]),
                                limits[1][6:], np.array([5, 5, 5])], axis=0)

        self.observation_space = spaces.Box(low=lower, high=upper)
        # initially: actions are joint targets
        self.action_space = spaces.Box(low=-np.ones(11),
                                       high=np.ones(11))

        # define goal

        self.table_cube = UrdfObject("cube_small.urdf",
                                     world_pos=[-0.75, 0.0, 0.25],
                                     fixed=True)
        self.goal_cube = UrdfObject("cube_small.urdf",
                                    world_pos=[-0.75, 0.0, 0.751],
                                    fixed=False)
    ...
    def observe_state(self):
        ...
        return np.concatenate([endeffector_pose, cube_pos, joint_pos[6:],
                               sensor_readings], axis=0)
    ...
    def calculate_reward(self, state, action, next_state, done):
        if next_state[9] < 0.5:
            ...
            return -0.1 * np.linalg.norm(next_state[7:9] - np.array([-0.5, 0.5]))
