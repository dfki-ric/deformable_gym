...
def test_sphere():
    simulation = BulletSimulation(time_delta=0.01, mode=pb.DIRECT, gravity=-9.81)
...
def test_urdf():
    ...
    try:
        obj = UrdfObject("plane.urdf", world_pos=(0, 0, 0), fixed=True, client_id=simulation.get_physics_client_id())
        pose = obj.get_pose()
        assert_array_almost_equal(pose, np.array([0, 0, 0, 1, 0, 0, 0]))
...
def test_insole():
    simulation = BulletSimulation(time_delta=0.01, mode=pb.DIRECT, gravity=-9.81, soft=True)
    simulation.timing.add_subsystem("time_step", 1, None)

    try:
        obj, _, _ = ObjectFactory().create("insole")
        # obj = Insole(insole_markers2world=np.eye(4), fixed=False, client_id=simulation.get_physics_client_id())
        pose = obj.get_pose()
        # assert_array_almost_equal(pose, np.array([0, 0, 0, 1, 0, 0, 0]))
