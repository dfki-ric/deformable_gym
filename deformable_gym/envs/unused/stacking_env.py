...
class StackingEnv(BaseBulletEnv):
    ...
    def __init__(
        ...
        lower = np.concatenate([np.array([-1.25, -0.25, 0.75]), np.zeros(4),
                                limits[0][6:], np.array([-5, -5, -5]), np.array([5, 5, 5, 5, 5, 5])], axis=0)
        upper = np.concatenate([np.array([-0.75, 0.25, 1.25]), np.ones(4),
                                limits[1][6:], np.array([5, 5, 5]), np.array([5, 5, 5, 5, 5, 5])], axis=0)
    ...
    def _load_objects(self):
        ...
        self.table = BoxObject(
            ...
            world_pos=[-0.5, 0.0, 0.5], world_orn=np.deg2rad([0, 0, 0]), mass=100,
            fixed=True)

        self.cube1 = BoxObject(
            half_extents=np.array([0.05, 0.05, 0.05]),
            world_pos=[-0.5, -0.1, 1.05], world_orn=np.deg2rad([0, 0, 0]), mass=1,
            fixed=False)
    ...
    def observe_state(self):
        ...
        return np.concatenate([endeffector_pose, joint_pos[6:], sensor_readings, cube1_pos, cube2_pos], axis=0)\
            .astype('float64')
    ...
    def calculate_reward(self, state, action, next_state, done):
        ...
        if cube1_pos[2] < 0.5 or cube2_pos[2] < 0.5:
            ...
            return -np.linalg.norm(self.cube1.get_pose()[:2] - self.cube2.get_pose()[:2]) * 0.1
